...
export default function Edit({auth,downtime}) {
    const {downtimeCategoryData} = useData();
    const { data, setData, put, processing, errors } = useForm({
        category: downtime.category,
        downtime:getFormatDate(downtime.downtime,"yyyy-MM-dd'T'HH:mm"),
        uptime:getFormatDate(downtime.uptime,"yyyy-MM-dd'T'HH:mm"),
    ...
    const submit = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                put(route(`admin.downtime.update`,downtime.id),{
                    _method: 'PUT',
                    ...data
                });
            }
        ...
        <Authenticated user={auth.user}>
            ...
            <div className="flex justify-center">
                <form onSubmit={submit} className="bg-white w-full md:w-1/2 shadow-md rounded-md flex flex-col overflow-hidden">
                    <div className="w-full px-5 py-2 font-semibold text-rose-600 border-b">Edit Downtime</div>
                    <div className="p-5">
